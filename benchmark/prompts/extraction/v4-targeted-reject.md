...
description: >
  ...
  v4: tighter rejection of one-off findings, implementation logs, and navigation details.
goal: >
...
- **Implementation decisions just made** — if the assistant just implemented or refactored something in this session, that is recorded in the code/commit, not an engram. Only extract if the learning applies *beyond* the current task.
- **Page/route/URL mappings** — "/page_a is feature X, /page_b is feature Y" is navigation documentation, not a reusable learning. Only extract if there is a non-obvious *gotcha* (e.g. "route A silently redirects to B when flag is off, causing test failures").
- **One-off debugging findings** — "on page X, data only shows when status === 'CONVERTED'" is a single investigation result. Only extract if the *pattern* is reusable (e.g. "API responses may contain data that the UI conditionally hides — check the raw response before assuming data is missing").
- **Hyper-specific data details** — column formats, field names, cache key structures, or data schemas for one particular project/spreadsheet/API are not reusable across tasks.
...
**Durability**: If the engram would become false or irrelevant after the current task is completed (a feature flag is removed, a refactor lands, a threshold is retuned), do not extract it.
...
Bad engrams (DO NOT extract these patterns):
...
- "The location field is empty for CITY profiles — use onsiteLocations array" (one-off data detail for one model)
...
- "The Fulfillment Requests page is at /internal_jobs; the Job Station is at /fulfillment" (route mapping — belongs in docs, not an engram)
- "On the DemandSignal page, data only shows when status === 'CONVERTED'" (one-off debugging finding for one page)
- "In via-dacica, names are stored as 'FirstName LASTNAME' in the Nume column" (hyper-specific data format for one spreadsheet)
- "We just removed the scope field from the extraction prompt and updated tests" (implementation log — the code/commit is the record)
