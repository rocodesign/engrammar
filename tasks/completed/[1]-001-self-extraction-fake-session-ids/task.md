...
MCP `engrammar_add` stores placeholder strings ("current-sess", "conversation", "engrammar-pl") instead of real Claude session IDs. ~20 engrams have fake IDs, ~10 have empty `source_sessions`.
...
4. **Backfill migration**: Match fake-ID engrams to real sessions by timestamp proximity (`engram.created_at` vs `session_audit.timestamp`).
