...
Haiku defaults to "figma" as catch-all topic. Engrams about cli.py, Serena, facets, prerequisites all get `tools/figma` category. 10+ engrams miscategorized.
...
2. **Add explicit instruction**: "Use 'general' if no specific topic fits. Never force a topic that doesn't match the engram content."
3. **Expand `TOPIC_CATEGORY_MAP`** with entries for common misses so even vague topics map to reasonable categories.
4. **Optional post-extraction validation**: If Haiku's returned topic doesn't appear in the engram text or any keyword map, remap to `general`.
