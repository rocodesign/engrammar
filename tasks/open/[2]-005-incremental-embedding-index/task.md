...
`build_index` recomputes embeddings for all active engrams on every add/update. O(n) in engram count, requires loading the embedding model each time. Overlaps with issue #12 but also covers text updates from refinement (#32).

## Fix

1. **`update_embedding(engram_id, text)`**: Embed just the changed text, find position in ID array, replace that row in the numpy matrix, save.
2. **`append_embedding(engram_id, text)`**: For new engrams, embed and append vector + ID rather than rebuilding.
